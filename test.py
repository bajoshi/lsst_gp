...
def mag_in_filter_line(filter, spectrum, redshift, ew):
    ...
    # calculate distances
    dp = 1e-3*2.99792458e8*cc.proper_distance(H_0, omega_m0, omega_r0, omega_lam0, 1/(1 + redshift))[0] * 1e6 * 3.08567758e18 # in cm
    dl = (1 + redshift) * dp
    ...
    for i in range(len(filter)):
        ...
        if f_lam != f_lam_noline:
            ...
            print flux_redshifted[lam_index-2:lam_index+3], flux_redshifted_noline[0][lam_index-2:lam_index+3]
...
gs.update(left=0.1, right=0.95, bottom=0.1, top=0.95, wspace=0.65, hspace=1.1)
...
z_range = [np.arange(1.6, 2.3, 0.1), np.arange(2.3, 3.4, 0.1), np.arange(3.4, 4.7, 0.1),\
           np.arange(4.7, 5.7, 0.1), np.arange(5.7, 6.5, 0.1), np.arange(7.6, 7.7, 0.1)]
